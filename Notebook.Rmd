...
rocr_pred_object <- ROCR::prediction(
  model %>% keras::predict_proba(x_train_validate) %>% as.vector(), # prediction probabilities
...
```{r}
keep <- c("x_test", "x_train", "x_train_train", "y_train_train", "x_train_validate", "y_train_validate")
...
model <- keras_model_sequential() %>% 
  layer_dense(units = 16, activation = "relu", 
              kernel_regularizer = regularizer_l1_l2(l1 = 0.001, l2 = 0.001), input_shape = dim(x_train)[2]) %>% 
  layer_dense(units = 16, activation = "relu", 
...
This step is included to show the *upload* step to participate in Kaggle competitions.

First, the *test data* was already prepared in the above section. It looks like this:
...
I can now export to csv and upload to: https://www.kaggle.com/c/santander-customer-transaction-prediction/submit.
